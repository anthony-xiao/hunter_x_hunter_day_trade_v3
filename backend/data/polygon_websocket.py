...
class PolygonWebSocketManager:
    ...
    async def connect(self) -> bool:
        ...
        try:
            self.client = WebSocketClient(
                ...
                feed="delayed",  # Use "real-time" for live data with appropriate subscription
                market="stocks"
            )
    ...
    async def subscribe_quotes(self, symbols: List[str]):
        ...
        try:
            # Subscribe to quotes
            await self.client.subscribe_quotes(*symbols)
            self.subscribed_symbols.update(symbols)
    ...
    async def subscribe_minute_aggs(self, symbols: List[str]):
        ...
        try:
            ...
            await self.client.subscribe_minute_aggs(*symbols)
            self.subscribed_symbols.update(symbols)
    ...
    async def _handle_quote(self, quote: EquityQuote):
        """Handle quote data"""
        try:
            # Use mid-price as the primary price
    ...
    def get_latest_price(self, symbol: str) -> Optional[float]:
        ...
        if symbol in self.latest_trades:
            ...
            return self.latest_quotes[symbol].price
        elif symbol in self.latest_aggs:
            return self.latest_aggs[symbol].price
        return None
